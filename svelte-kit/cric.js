...
const verify = async () => {
    ...
    let { stdout, stderr } = await execPromise('cd $HOME/workspace/myProjects/js/svelte-kit/svelte-kit-cric && source .envrc && (grep \'PORT=\' .envrc | awk -F= \'{print $2}\')');
    const port = parseInt(stdout);

    const deployResponse = await execPromise('cd $HOME/workspace/myProjects/js/svelte-kit/svelte-kit-cric && source .envrc && zsh deploy.sh');
    ...
    const stopResponse = await execPromise('cd $HOME/workspace/myProjects/js/svelte-kit/svelte-kit-cric && source .envrc && zsh stop.sh');

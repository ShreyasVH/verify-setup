...
const verify = async () => {
    ...
    let { stdout, stderr } = await execPromise('bash -c "cd $HOME/workspace/myProjects/js/solid/solid-cric && source .envrc && (grep \'PORT=\' .envrc | awk -F= \'{print $2}\')"');
    const port = parseInt(stdout);

    const deployResponse = await execPromise('bash -c "cd $HOME/workspace/myProjects/js/solid/solid-cric && source .envrc && bash deploy.sh"');
    ...
    const stopResponse = await execPromise('bash -c "cd $HOME/workspace/myProjects/js/solid/solid-cric && source .envrc && bash stop.sh"');
